...
var config = {
	name: 'fcZljk',
	version: 'v1.0.0',
	oss: `http://image.fcyun.com/fcZljk/`,
	replaceFileArr: [],
	projSrc: `F:/workspace/zljk/zljk-admin/src/main/resources`
};
...
gulp.task('_rev_assets', done => {
	return gulp.src([path.resolve(config.projSrc, './static/fonts/web-icons/*.*'), '!' + path.resolve(config.projSrc, './static/fonts/web-icons/*.css'), path.resolve(config.projSrc, './static/**/*.png'), path.resolve(config.projSrc, './static/**/*.gif'), path.resolve(config.projSrc, './static/**/*.jpg'), path.resolve(config.projSrc, './static/**/*.svg')])
		.pipe(rev())
...
gulp.task('_html', ['_js'], function (done) {
	return gulp.src(['./dist/rev/**/*.json', './dist/templates/**/*.html'])
		...
		//替换css 里面的路径
		.pipe(gulpif('*.css', replace('/static', '/static/' + config.version)))
		//替换html 里面的路径
		.pipe(gulpif('*.html', replace(/.*?static.+/g, function (str) {
			if (str.indexOf('<img') > -1 || str.indexOf('rel="icon"') > -1 || str.indexOf('<meta') > -1 || str.indexOf('rel="apple-touch-icon-precomposed"') > -1) {
				if (str.indexOf('src=') > -1) {
					...
					str = str.replace(src, src.replace(/^.*?static\//, '/static/' + config.version + '/'));
				} else if (str.indexOf('href=') > -1) {
					var href = str.match(/href\=\"(.*?)\"/)[1];
					str = str.replace(href, href.replace(/^.*?static\//, '/static/' + config.version + '/'));
				} else if (str.indexOf('content=') > -1) {
					var content = str.match(/content\=\"(.*?)\"/)[1];
					str = str.replace(content, content.replace(/^.*?static\//, '/static/' + config.version + '/'));
				}
			} else if (str.indexOf('modules/') > -1) {
				str = str.replace('/modules', '/' + config.version + '/modules')
			}
		...
		.pipe(revReplace({
			modifyReved: function (filename) {
				if (filename.indexOf('.html') > -1) {
					...
					filename = '/static/' + filename.replace(/^.*?build\//, '')
				}
				return filename;
			}
		}))
